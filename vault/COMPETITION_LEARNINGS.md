...
| L22 | craft / render-survivability | Suno | music | Production-spec answers DO NOT SURVIVE THE RENDERER — only lyric and form do | HIGH (measured 2026-07-24) | First render-verified audit in the ledger. Two finished Suno tracks decoded and measured: the new publish and a proven prior publish. **Not one production-level gesture survived.** (1) *The hollow centre*: spec called for the take to stop mid-strum for ~4 seconds with only two drones audible; the deepest dip in the entire middle third measures **5.7 dB over 0.40 s**. Smoothed into a musical breath — exactly the failure the pair predicted and wrote three separate mitigations against. (2) *The somatic counter-move*: a tuned strip-light mains hum and a drive-motor whine a quarter-tone apart, "nothing corrects either" — **zero isolated sustained tonal components** in any channel (≥12 dB local prominence, ≥50 cents separation). It did not render at all. (3) *Spatial staging*: "hard left / hard right / dead centre / far back" — **both tracks render essentially MONO** (L-R correlation 0.945 and 0.908; side/mid 0.17 and 0.22). **Consequences, binding:** (a) every spatial instruction in every music prompt is decorative — it costs characters and buys nothing; (b) any hollow centre whose mechanism is *"the mix collapses to mono and distant"* is **unfalsifiable**, because the render is already mono; (c) **a Somatic-Gate objection answered in the production spec is NOT answered** — on 2026-07-24 a `REPAIR — 2/3` was closed with a mix decision (the quarter-tone drone pair), and that answer never reached audio. **The objection is still open in the only place that counts.** Answer distinctiveness objections in the LYRIC and the FORM, which survive; treat production spec as flavour for the model, never as the mechanism a song depends on. *Counter-evidence worth holding: the track lost all three gestures and the human still called it amazing — the writing carried it alone, which is the strongest available argument for putting the load there.* |
