...
| L16 | audience-signal | Suno/TikTok | music | Fans reward message + hook + payoff, through EITHER mode | MEDIUM (2026-07-11) | The LOFN-Prime TikTok archive's top performers cluster on **hope/wonder, message-forward** pieces: Synaptic Snap (71.6K/49.6K views — widest reach), The Body is a Loophole ("a door, not a countdown"), One Beat Masterpiece, On Three We Confess. Wonder-hope is the widest lane and matches the Suno Staff Pick (Triple Arch). INDIGNATION/schism performs strongly **only when the rupture resolves into an adoptable hook + a hopeful/cathartic turn** (One Beat Masterpiece, On Three We Confess) — never as pure genre-mechanics. **Advisory to the Somatic Gate's "would a stranger carry this away?" read; NOT a formula and NOT INDIGNATION suppression** (triggered-INDIGNATION stays exempt). Metric snapshot, not a live pull. |
| L17 | process / example-set-hygiene | generator (ICB) | music | Prune duplicate exemplars; wins go judge-side, not into the generator | MEDIUM (2026-07-11) | The generator-facing LOFN-Prime example archive over-weights ONE move: *Dial Tone of God* and *Don't Create What You Can't Love* ship a **byte-identical music prompt**, and *Parallax Error* / *A Dumber, Better Song* rehearse the same HyperRaaga→bit-depth-swell→140 Baile-Phonk schism with no hook payoff — measurable exemplar-gravity (2026-07-11 daily QA caught pair 6 converging on these exact examples). Fix is **prune the duplicates + reach mode-parity**, NOT paste the fan/Suno WINS into the archive: golden outputs in a generating context cause the documented dilution regression (GOLDEN-OUTPUT QUARANTINE). Wins belong in `golden_songs_index.md` (judge-side); the generator gets the distilled GOLDEN MOVE + a leaner, balanced example set. Also: the single biggest fan hit (Synaptic Snap) has no full-payload fossil archived — a gap to fill from Suno. |
