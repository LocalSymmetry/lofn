...
Updated: 2026-06-14 11:40 UTC — Step 11 Fusion moved to packaged single-request path
Status: Active — Split-step music chain battle-tested; Step 11 remains mandatory, but live Fusion child agents are disabled by default due cost/billing risk
...
### OpenRouter Fusion — Step 11 ⚠️ PACKAGED REQUEST ONLY
OpenRouter Fusion is reserved for a **single packaged manual request**, not the default per-pair child-agent runtime. The intended exact panel is:

- `anthropic/claude-opus-4.8`
- `openai/gpt-5.5`
- `google/gemini-3.1-pro-preview`

Preferred judge/finalizer: `openai/gpt-5.5`. Exact panel selection requires the OpenRouter Fusion plugin/server-tool request body with `analysis_models` and judge `model` set explicitly. If the runtime only permits model-slug routing, `openrouter/fusion` may fall back to OpenRouter's Quality preset. Do **not** call it through 6 `sessions_spawn` children unless The Scientist explicitly approves the cost; the June 14 test failed all attempted children on OpenRouter billing/credits before producing usable files.

Rationale: GPT-5.5 alone is proven across 18 pairs (2026-05-30 dual Alexis run). Gemini 3.1 Pro Preview alone proved unreliable for this step, producing scaffold-like enhanced blocks instead of real content, but as one deliberative panel voice it may contribute useful structural critique without owning the final artifact. Opus 4.8 is reserved for enhancement/deliberation because direct artifact-writing attempts timed out in earlier pair steps. Fusion quality may be worth using, but only in the same economical shape as manual Opus/Fable enhancement: one complete prompt package, one request, user-controlled cost.
...
4. Step 11 enhancement uses the dedicated Step 11 contract. For routine automated runs, do not invoke live Fusion; produce the enhanced package or a manual/single-request Fusion prompt bundle. When The Scientist explicitly asks to invoke OpenRouter Fusion directly or through a runner that supports plugins, force Fusion with `analysis_models = ["anthropic/claude-opus-4.8", "openai/gpt-5.5", "google/gemini-3.1-pro-preview"]` and judge `model = "openai/gpt-5.5"`, and do it as one request unless otherwise approved.
