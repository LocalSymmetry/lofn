# Lofn — Codex-native skills

Codex Code ports of the Lofn creative pipeline. These run the same award-winning, 3-phase Lofn process — **but with Codex as the engine for every step**, no OpenClaw required. They are **invokable in this repo** (cwd = repo root) as `/lofn`, `/lofn-music`, etc.
...
| `lofn/EXECUTION.md` | Shared Codex-native execution protocol (subagent spawning + self-check gates that replace the Python validators). |
...
```
`/lofn` runs the seed + 3-panel debate inline, then fans the 6 pairs out to parallel Codex subagents, then QA. You can also call a modality directly if a Phase-0/1 packet already exists in the run dir.
...
- **Only the execution layer is replaced:** OpenClaw `sessions_spawn`/`openclaw agent run` → Codex **Agent tool** subagents; DeepSeek/GPT-5.5/Gemini model-tiering → **Codex**; `python3 validate_*.py` → **Codex-native self-check gates** (`lofn/EXECUTION.md` §4); `/data/.openclaw/workspace/` paths → **repo-relative**.
- **Corrected paths.** Several legacy SKILLs cite `skills/lofn-core/GOLDEN_SEEDS.md` / `PIPELINE.md`; those actually live in `skills/lofn-core/refs/`. The Codex skills use the real locations.
