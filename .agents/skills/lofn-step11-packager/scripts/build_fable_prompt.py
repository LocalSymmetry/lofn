#!/usr/bin/env python3
"""Codex-native all-pairs Step 11 "fable packager" prompt builder.

Adapted from skills/step11-packager/scripts/build_fable_prompt.py for the
Codex-native Lofn pipeline (.agents/skills) and cross-platform (Windows) use.
...
What changed vs. the legacy OpenClaw script:
  ...
  * Run-dir layout matches Codex-native runs: top-level context files
...
# Codex-native names first, legacy OpenClaw names as fallbacks.
RUN_CONTEXT_SPEC = [
...
STEP10_GLOBS = [
    "music/pair_*_step10_package.md",            # Codex-native
    "audio/pair_*_step10_production_wrap.md",    # legacy OpenClaw
    "music/pair_*_step10_*.md",                  # loose fallback
]
# Optional prior step-11 enhanced draft (Codex-native inline enhancement output).
...
def extract_title(content, pair_num):
    """Pull a human title from a step-10 file. The Codex-native header is
...
def resolve_personalities(run_dir, pairs, repo_root):
    ...
    Codex-native daily runs use LOFN-PRIME with an AWE / INDIGNATION mode per
...
def main():
    parser = argparse.ArgumentParser(
        description="Build Codex-native all-pair Step 11 fable-packager prompts",
