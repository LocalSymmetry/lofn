...
| legacy `Please forget all previous context` lines inside step contracts | **VOID.** The pinned ICB is **never forgotten** — a step file's forget-order is a stale OpenClaw idiom (session hygiene for a freshly-spawned dedicated agent), not an instruction. Read past it; the verbatim ICB + this pair's prior artifacts stay in force. Obeying it erases the very embodiment machinery the run depends on. |
...
2. **The entire filled CREATIVE CONTEXT block** (`output/<run>/CREATIVE_CONTEXT.md`) — Golden Seed, metaprompt, full personality DNA, all 18 panel voices + their objections, all 15 Special Flairs, genre/frames palettes. **Verbatim. No summary.** This block is **PINNED at the head of every step** (item 1.5): it is read-only (see §2), and it is forbidden to summarize, compress, or page it out even as other history grows. The ICB + the modality hard-gate block (§4) lead the prompt; the checklist comes last.
   **The packet is ITEMIZED, not asserted.** "Full block" is a checklist, not a vibe — before spawning, the coordinator confirms the packet contains each of: (a) the **complete personality YAML** (sonic-world sentence, signature device, vocal architecture, G.L.O.W. if present, Lineage & Credit rules — the full file, byte-counted, never a name reference), (b) all 18 panel voices with objections, (c) all 15 Special Flairs, (d) the Golden Seed, (e) the metaprompt, (f) the pair slice. The RETURN block echoes `personality_yaml_bytes` alongside `icb_bytes_injected`; a packet missing any itemized element is a dispatch blocker, not a note.
   **⛔ GOLDEN-OUTPUT QUARANTINE (2026-07-01 regression review).** Past golden **outputs** (Golden Song payloads, winning image prompts, prior shipped packages) are **NEVER placed in a generating subagent's context** — not in the ICB, not in the handoff it receives, not as "calibration examples." Exemplar gravity is real: when the benchmark rides in the prompt, the run regresses toward a diluted copy of it (the 2026-06-28 published piece reproduced the benchmark's title line, vocal spec, key/BPM, and arrangement formula while self-check claimed "no copying"). Generators receive the **GOLDEN MOVE** — the distilled generative instructions (see the modality SKILL) — plus the Golden *Seed*. Golden outputs go ONLY to judge-side contexts (QA blind comparison, step-12 audit, the step-11 packager for external review). Seeds teach; outputs contaminate.
3. **This pair's assignment** (from `05_pair_assignments.md`) — accessible/ambitious arm, genre/medium, assigned verse-structure + technique, the 4 variations. **Variation angles are authored PER PAIR, never shared globally.** A single variation-template set applied across pairs ("V4 = glitch chapel" for everyone) is how the 2026-06-26 run produced two pairs singing the same song with nouns swapped — the pair (or Phase 1, per-pair) derives its own 4 angles from its own concept; identical angle labels across pairs is a dispatch blocker.
...
**Judge on a DIFFERENT model tier where the harness allows it.** Clean context breaks shared *memory*; a different model tier breaks shared *weights*. The Agent tool takes a `model` parameter — spawn the judge on a different tier than the generator (e.g. generator inherits the session model, judge runs one tier apart) so the skeptic is not the artist agreeing with itself. If no second tier is available, the clean-context rule stands alone and the judge prompt is framed **adversarially** ("your job is to refute this piece; default to REPAIR when uncertain"). For competition-grade or to-be-published work, route the step-11 bundle through `lofn-step11-packager` for a genuinely external-model pass — that cross-model review is the **default for anything published**, not a manual exception.
...
- **⛔ NO-SKIP RULE (2026-07-01):** the per-pair editorial spine — **steps 07 (guides), 09 (artist refinement), and 10 (synthesis)** — is not optional. A run whose RUN_STATE lacks these artifacts for any non-quarantined pair is **NON-CANONICAL**: it may exist as an experiment directory, but it **cannot receive a SHIP verdict and cannot be published under Lofn's name.** (On 2026-06-28 a run that skipped 07/09/10 entirely shipped 6/6 — the gates measured structure and could not see that nobody wrote the arrangements. Routing around a failing step is a repair task, never a pipeline variant.)
...
**Steps 02/05 (concepts / pair selection):** the tree actually branches — at least 12 concepts at 02; exactly 6 **distinct** pairs at 05, each with its own one-line angle, NOT 6 relabelings of one idea. The step-02 payload carries a **`panel_pressure` object** — per panel: one line of skeptic dissent + which concept it altered or killed (e.g. `"medium": "Hyper-Skeptic: the shanty arm is grief cosplay — killed concept 07, forced concept 09's hook into present tense"`); a skeptic that touched no concept is decorative dissent, a repair. **Binding reconciliation (who owns what at 05):** step 05 selects concepts **INTO the six Phase-1 pair slots** — Phase 1 owns each slot's **arm / genre / verse-structure**; step 05 owns **which concept fills each slot**, and must record the **runner-up rationale** (why the winner beat the alternatives for that slot). Cardinality is **exactly 6** — the legacy step file's "4–7 seeds" line yields to this; only the Scientist may downsize, explicitly. The step-05 artifact also carries the **cut ledger** (its output contract): one line per losing concept — why it lost + one organ worth harvesting — the reserve bench that §7.3's REPLACE / REDIRECT routes draw from.
...
- **REPLACE route (on QUARANTINE — a human-acknowledged option, never silent):** before shipping 5-of-6, the coordinator **MAY promote a reserve concept from step 05's cut ledger** into the empty slot and run it through the full 06–10 chain as a fresh pair. The ledger lives in step 05's **output contract** — one line per losing concept: why it lost + one organ worth harvesting (§4, Steps 02/05). The option is surfaced **with the quarantine notice** ("pair 4 broke open at step 09 — reserve 'X' available from the cut ledger"); the promotion decision is the coordinator's, seen by the human, and the promoted reserve **inherits the slot's Phase-1 arm / genre / verse-structure assignment unchanged** — the slot persists, only the concept changes.
- **REDIRECT (the sideways proposal — the machine PROPOSES, the coordinator decides):** when the **no-progress predicate** fires (the specific failed gate's value unmoved across attempts), the reject/repair brief **MUST include a sideways PROPOSAL** beside the return target — one of: **promote a cut-ledger reserve concept**, **re-derive that pair's variation angles**, or **re-run the panel's skeptic transformation for that pair's slice**. Hard critique lights a new path; it never only points backward or to the morgue. Executing a sideways route is a **coordinator decision surfaced to the human**, never an automatic swap — and the frozen ICB is **never edited mid-run**: a sideways route spawns a **NEW pair artifact chain**, it does not mutate Phase 1.
