...
def generate_concept_mediums(input, max_retries, temperature, model="gpt-3.5-turbo-16k", verbose=False, debug=False, aesthetics=aesthetics):
    ...
    for key in [
        ...
        'medium_output', 'refined_medium_output', 'shuffled_review_output', 'creativity_spectrum'
    ]:
        if key not in st.session_state:
            st.session_state[key] = None
    ...
    st.session_state.creativity_spectrum = st.session_state.essence_and_facets_output["essence_and_facets"]["creativity_spectrum"]
    if debug:
        st.write(f"creativity_spectrum: {st.session_state.creativity_spectrum}")
    ...
    st.session_state.concepts_output = parse_output(concepts_schema, run_chain('concepts', {
        ...
        "creativity_spectrum_wild": st.session_state.creativity_spectrum['wild'],
        "creativity_spectrum_creative": st.session_state.creativity_spectrum['creative'],
        "creativity_spectrum_grounded": st.session_state.creativity_spectrum['grounded'],
    }), 
    debug, 
    max_retries=max_retries)
