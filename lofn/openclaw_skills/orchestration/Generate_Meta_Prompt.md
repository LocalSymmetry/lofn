...
## Description
Synthesizes the competition requirements and persona into a high-level meta-prompt that drives subsequent creative generation.

## Trigger Conditions
- Use this skill when assigned a Kanban task to create a meta-prompt, after the persona has been defined.

## Required Inputs
- `[input]`: The competition text, optional image context, and the active persona.
...
- Return the meta panel prompt in the following JSON format:
  ```json
{{ "meta_prompt": "The written meta-prompt." }}
  ```
