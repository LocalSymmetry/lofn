...
def generate_dalle_images(input, concept, medium, df_prompts, max_retries, temperature, model, debug):
    ...
    for index, prompt in enumerate(all_prompts):
        ...
        if image_url:
            ...
            filename = f"{timestamp}_{concept}_{medium}_{prompt_type}_{index + 1}.png"
...
def save_metadata(metadata):
    ...
    metadata_filename = f"/metadata/{metadata['timestamp']}_{metadata['concept']}_{metadata['medium']}_{metadata['prompt_type']}_{metadata['prompt_index']}.json"
...
with st.container():    
    if st.session_state['concept_mediums'] is not None:
        ...
        if st.button("Generate Image Prompts"):
            st.write("Generating prompts for selected concept-medium pairs")
            selected_pairs = [st.session_state['concept_mediums'][i] for i in st.session_state['pairs_to_try']]
            all_prompts = generate_all_prompts(st.session_state.input, selected_pairs, max_retries, temperature, model, debug)
            
            for pair, prompts in zip(selected_pairs, all_prompts):
                if prompts is not None:
                    st.write(f"Prompts for Concept: {pair['concept']}, Medium: {pair['medium']}")
                    ...
                    st.error(f"Failed to generate prompts for Concept: {pair['concept']}, Medium: {pair['medium']}")
        else:
            st.write("Ready to generate Image Prompts")

    if st.session_state['concept_mediums'] is not None and model != "gpt-4":
        if st.button("Generate All"):
            st.write("Generating prompts for all concept-medium pairs")
            all_prompts = generate_all_prompts(st.session_state.input, st.session_state['concept_mediums'], max_retries, temperature, model, debug)
            
            for pair, prompts in zip(st.session_state['concept_mediums'], all_prompts):
                if prompts is not None:
                    st.write(f"Prompts for Concept: {pair['concept']}, Medium: {pair['medium']}")
                    st.dataframe(prompts)
                    dalle_prompt = dalle3_gen_prompt if enable_diversity else dalle3_gen_nodiv_prompt
