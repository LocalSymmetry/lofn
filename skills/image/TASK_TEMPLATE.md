...
- Step 00: read `steps/00_Generate_Image_Aesthetics_And_Genres.md`, call the model, write `step00_aesthetics_and_genres.md` using the canonical provenance template. Provenance must cite the validated orchestrator packet
- Step 01: read `steps/01_Generate_Image_Essence_And_Facets.md`, call the model using Step 00 output, write `step01_essence_and_facets.md` using the canonical provenance template
- Step 02: read `steps/02_Generate_Image_Concepts.md`, call the model using Step 01 output, write `step02_concepts.md` using the canonical provenance template
- Step 03: read `steps/03_Generate_Image_Artist_And_Critique.md`, call the model using Step 02 output, write `step03_artist_and_critique.md` using the canonical provenance template
- Step 04: read `steps/04_Generate_Image_Medium.md`, call the model using Step 03 output, write `step04_medium.md` using the canonical provenance template
- Step 05: read `steps/05_Generate_Image_Refine_Medium.md`, call the model using Step 04 output, write `step05_refine_medium.md` using the canonical provenance template and `concept_medium_pairs.json` (6 pairs)
...
- Step 06: read `steps/06_Generate_Image_Facets.md`, call the model, write `pair_{NN}_step06_facets.md` using the canonical provenance template. Provenance must cite the Golden Seed, orchestrator metaprompt, pair assignment, Step 05 concept-medium pair, and prior coordinator outputs
- Step 07: read `steps/07_Generate_Image_Aspects_Traits.md`, call the model using Step 06 output, write `pair_{NN}_step07_aspects_traits.md` using the canonical provenance template
- Step 08: read `steps/08_Generate_Image_Generation.md`, call the model using Step 07 output, write `pair_{NN}_step08_generation.md` using the canonical provenance template
- Step 09: read `steps/09_Generate_Image_Artist_Refined.md`, call the model using Step 08 output, write `pair_{NN}_step09_artist_refined.md` using the canonical provenance template
- Step 10: read `steps/10_Generate_Image_Revision_Synthesis.md`, call the model using Step 09 output, write `pair_{NN}_step10_revision_synthesis.md` using the canonical provenance template
...
Returns: Step 10 final prompts as output text
