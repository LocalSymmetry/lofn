...
6. 🚨 **Lyrics field < 5000 chars HARD CAP (target ≤4800) — Suno render limit.** The field is everything pasted into Suno's lyrics box: `[Theme]` + `[SONG FORM]` + the Disc_Channel block + every EMO/section header + every `*SFX*` + all sung lines. **Measure the exact count; never estimate.** Over 5000 = will not render. To fit: trim/merge sung lines, tighten headers, then move the Disc_Channel block to a production sidecar *outside* the field. The 70–120-line target yields to this cap. Enforced by `scripts/validate_suno_packages.py`.

**Self-check must verify:** paragraph style prompt present, separate Suno exclude prompt present, Theme + SONG FORM present, Disc_Channel headers present, EMO-tagged section headers, no categorized key:value brackets in style prompt, no avoidances in style prompt, **lyrics field measured < 5000 chars (target ≤4800) — state the exact count**.
