...
4d. **Step 11 - Enhancement (DISC_CHANNEL MANDATE, 2026-06-09; FUSION BACKEND UPDATE, 2026-06-14):** After all pair agents complete Step 10, spawn enhancement agents (1 per pair, 5 concurrent max) using `lofn-audio-step11` per `vault/LOFN_MODEL_ASSIGNMENTS.md`. Each reads its Step 10 output + coordinator context + `vault/SUNO_PROMPT_CONSTRUCTION_GUIDE.md` + the 15-point QA checklist. Produces `pair_0X_step10_final_package_enhanced.md`. Reference: `steps/11_Generate_Music_Enhancement.md`. Current backend: `openrouter/fusion`. Exact intended Fusion panel: `anthropic/claude-opus-4.8`, `openai/gpt-5.5`, `google/gemini-3.1-pro-preview`; judge/finalizer: `openai/gpt-5.5` when plugin routing is available. Timeout: 300-900s each. This is now a permanent pipeline step - do not skip it.
...
**Style prompt:** Dense paragraph form, 850-1000 chars. Genre × Subgenre, BPM, key, 432Hz, vocalist specifics, instrumentation, arrangement arc, signature device, avoidances. NO categorized brackets. NO artist names. NEVER blank or single-word.
...
**Self-check must verify:** paragraph style prompt present, Theme + SONG FORM present, Disc_Channel headers present, EMO-tagged section headers, no categorized key:value brackets in style prompt, lyrics ≤5000 chars.
...
9. **FULL CONTEXT PAIR-AGENT INJECTION (MANDATORY 2026-06-10):** After Step 05, every pair agent spawn MUST receive the COMPLETE upstream context — no compression, no summarization, no skipping. This replaces the former "lean" standard entirely. The long prompts are why we won. Each pair agent spawn must include: (a) FULL personality DNA block for that pair's assigned personality — core identity, sonic world, signature device, catchphrase, G.L.O.W. Protocol, vocal architecture; (b) the pair-specific Golden Seed excerpt from the handoff file; (c) all Production Mandates; (d) the Forbidden Substitutions table; (e) the relevant pair assignment excerpt; (f) the step contract. The parent spawns the pair agent with this full payload injected into the task prompt. Do NOT rely on the agent reading upstream files — inject directly into the spawn task. A name reference like "voice = Eager Archivist" is INSUFFICIENT — the full DNA block must be present. The former `vault/LEAN_PAIR_AGENT_INPUT_STANDARD.md` is DEPRECATED by this rule.
...
- Do not collapse Steps 06-10 into one pair-agent prompt/response. Original Lofn calls each step sequentially; OpenClaw runs must mirror that granularity with separate artifacts and dedicated configured step agents per `vault/LOFN_MODEL_ASSIGNMENTS.md`. Step 11 (enhancement) runs after all pair Step 10 packages are complete — 1 agent per pair, currently `lofn-audio-step11` / `openrouter/fusion`, producing `pair_0X_step10_final_package_enhanced.md` in Disc_Channel format (dense paragraph style prompt, Disc_Channel header block, Theme + SONG FORM, full EMO section headers, no categorized key:value brackets in the style prompt). Do not skip Step 11. Step 12 (panel audit) runs when triggered, using `lofn-audio-step12` / `deepseek/deepseek-v4-pro`. If outputs look like one-shot backfilled files, missing required headers, or contradicted self-checks, QA must fail even when filenames exist.
