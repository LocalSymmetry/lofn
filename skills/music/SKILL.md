...
4d. **Step 11 - Enhancement (DISC_CHANNEL MANDATE, 2026-06-09; FUSION PACKAGED REQUEST UPDATE, 2026-06-14):** After all pair agents complete Step 10, Step 11 remains mandatory, but **do not spawn live `openrouter/fusion` agents by default**. Fusion is too costly as 6 separate child sessions and can fail the whole lane on billing. Default behavior is to produce Step 11 enhanced packages with the configured non-Fusion path, or to package a **single paste-ready Fusion request** covering the target pair/run when The Scientist explicitly asks for Fusion. Reference: `steps/11_Generate_Music_Enhancement.md`. Exact intended Fusion panel for manual/single request use: `anthropic/claude-opus-4.8`, `openai/gpt-5.5`, `google/gemini-3.1-pro-preview`; judge/finalizer: `openai/gpt-5.5` when plugin routing is available. Never launch 6 Fusion children unless The Scientist explicitly approves the cost.
