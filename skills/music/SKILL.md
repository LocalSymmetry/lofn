...
- **Required Suno prompt shape:** target 850-1000 characters (hard max 1000 unless the destination explicitly allows longer), no artist names, dense producer-grade language: emotion → selected style label(s) from the run → vocalist spec → instrumentation/mix → chronological progression → bold sonic device → avoidances. The Poe panel standard adds: do not pad with generic tag soup; if the prompt needs extra intelligence, route it into Sonic Manifest / Production Cathedral sidecars while keeping the copy-paste prompt producer-grade.
