...
4d. **Step 11 - Enhancement (DISC_CHANNEL MANDATE, 2026-06-09; FUSION DIRECT-CALL UPDATE, 2026-06-14):** After all pair agents complete Step 10, Step 11 remains mandatory, but **do not spend `openrouter/fusion` by default**. Default behavior is to produce Step 11 enhanced packages with the configured non-Fusion path, or to package paste-ready Fusion prompts for user-controlled use. When The Scientist explicitly approves Fusion invocation, call Fusion **six times, one isolated request per pair**, rather than one blended all-pairs prompt; avoid the old persistent Step 11 agent loop and use the leanest available direct/model-wrapper call. Reference: `steps/11_Generate_Music_Enhancement.md`. Exact intended Fusion panel: `anthropic/claude-opus-4.8`, `openai/gpt-5.5`, `google/gemini-3.1-pro-preview`; judge/finalizer: `openai/gpt-5.5` when plugin routing is available.
