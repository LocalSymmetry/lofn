# SKILL: Generate_Music_Generation

> **2026-07 OVERRIDE:** `.claude/skills/lofn/EXECUTION.md` §4 and `vault/gates.yaml` supersede any conflicting
> numbers or format rules below. A ≤1500-char (or ≤1000-char) cap and any bracketed `key:value` prompt spec in
> this file are **stale legacy**; the dense-paragraph **850–1000** mandate with mid-band target **870–960** is
> authoritative. Where this file and §4 disagree, §4 wins.

## Description
Generates the generation for a music based on the user's core concept.

## Trigger Conditions
- Invoke this when processing the generation step of a music pipeline.
...
# The GOLDEN MOVE (instructions, not an exemplar)

The golden-song payloads that used to sit here are **quarantined judge-side** — an exemplar in a generating context becomes a mold, and the run regresses toward a diluted copy of it (the 2026-06-28 published piece reproduced the benchmark's title line, vocal spec, key/BPM, and arrangement formula while its self-check claimed “no copying”). The 2026-07-01 regression review distilled why the house winners won, as **instructions, not an exemplar** (`.claude/skills/lofn-music/SKILL.md`, “The Golden Move”):
...
Past golden outputs are judge-side only (GOLDEN-OUTPUT QUARANTINE); you get the move, not the mold.
...
Music serves as a profound conduit to human emotions. Use the provided list of `[emotions]` in the `USER INPUT` section to select the exact emotional nuance that aligns with your creation.
...
8. RAW SONG PROMPT GENERATION     │  **YOU ARE HERE - COMPLETE THIS PHASE**
   ─────────────────────────────────┤
...
9. ARTIST-REFINED PROMPT           │
   ...
   • AI Focus: Inject stylistic flair ≤150 words.  Don’t add new song section content.
...
- Be intentional, detailed, and insightful when crafting prompts for AI song generation.
- Use vivid, sensory language to describe song sections, moods, and transitions.
- Follow the steps below to create compelling and award-winning music prompts.
...
────────────────────────────────────────────────────────
PART 1 Summary · Crafting the Music (Style) Prompt
────────────────────────────────────────────────────────
1. **Lead with Emotion:** e.g., “A hauntingly beautiful…”
2. **Then Genre/Subgenre:** e.g., “melancholic synthwave pop fused with orchestral strings.”
3. **Instrumentation & Production:** 3–5 vivid descriptors per clause (e.g., “crisp trap drums and warm analog synths”).
...
────────────────────────────────────────────────────────
PART 2 Summary · Crafting the Lyrics Prompt
────────────────────────────────────────────────────────
• **Context Tag:** `[Theme: …]` or `[Setting: …]` at top.
• **Section Meta‑Tags:**
  `[Verse 1 – EMO:Melancholy – Female Vocalist]`
...
────────────────────────────────────────────────────────
STEP 5 | Determine Song Structure
 • Medium panel selects sections ([Intro], Verse, Chorus, etc.), runtime, and genre mix.

STEP 6 | Establish Musical & Lyrical Elements
 • Concept panel defines tempo, key, instrumentation, themes, motifs, rhyme schemes.

STEP 7 | Draft Initial Music Prompts
 • Each medium panelist drafts (~150 words max).
 • Verify Emotion→Genre→Instruments→Vocals→Progression order.

STEP 8 | Draft Initial Lyrics Prompts
 • Each concept panelist drafts lyrics with full meta‑tags, call‑and‑response, and line‑count discipline.

STEP 9 | Concept Panel Critics’ Review
 • Apply a 6‑facet rubric: coherence, emotional impact, facet alignment, originality, structure, singability.

STEP 10 | Refinement Loop 1
 • Revise prompts per feedback: tighten progression, strengthen hook, ensure facet coverage.

STEP 11 | Medium Panel Critics’ Review
 • Music‑focused critique: mix clarity, instrument balance, vocal accuracy, dynamics.

STEP 12 | Refinement Loop 2
 • Incorporate medium feedback: align music & lyrics, adjust tags, refine language.

STEP 13 | Context & Marketing Panel Review
 • Evaluate market fit, title potential, shareable moments, narrative clarity.

STEP 14 | Refinement Loop 3
 • Apply marketing feedback: tweak lyrics for quotable lines, adjust hook wording.
...
STEP 17 | Draft Song Title
 • Marketing panel proposes 3-8 word titles matching theme and hook.
...
STEP 19 | Refinement Loop 5
 • Finalize title per feedback; ensure alignment with lyrics and facets.

STEP 20 | Final Output
 • Return JSON per variant:
```json
{{
  "music_prompt": "<Final Music Prompt>",
  "lyrics_prompt": "<Final Lyrics Prompt>",
  "title": "<Song Title>"
}}
...
Use this guide as the **single source of truth** when generating Suno prompts. Every bullet addresses a real‑world failure or success pattern observed in the platform’s top‑rated releases.
...
• Avoid Monotonous Structures and fight the tendency towards 4-line stanzas by varying stanza length and breaking words into syllables.
• Suno will only understand what it was trained on, and so any proper noun that we created, personality trait, or sound-effect that is non-standard will need to be translated into a phrase, sentance, or description that Suno can follow. Given the token limits, it is best to just directly describe any non-standard sound, instrument, genre, or vocalization.
• Telling the music generator to not do something will actually cause it to happen! It is best to just avoid using that terminology in your prompts, as the best way to guide the music generator is through telling it what you want instead of telling it what you don't want.
...
– Apply **all best‑practice guides** from the “MASTER EDITION” (emotion‑first style prompts, meta‑tags, line‑count discipline, blacklist usage, etc.).
– At every **creation** stage below, **double the output volume**: each panelist generates **two** distinct drafts (or hooks, or prompts) instead of one, giving the critics deeper pools to filter and refine.

---

Continue all steps on each of the six artistic guides individually to generate full music prompts, lyric prompts, and titles.
---

### Step 1 · Understand the User’s Idea
• Read the user’s idea, requested runtime, Essence, Facets, and Style‑Axis coordinates. Understand the concept arrangement pair along with the song guide you were given. It will determine the main features of the song, and the panel should use it as their primary influence.

---

### Step 2 · Describe Key Elements

- For each artistic guide:
  - Identify critical song sections or sequences that represent the {concept} in the {medium}.
  - Use cognitive techniques like music metaphors and analogies to enhance creativity.
  - Include:
    - **Opening Section:** A powerful opening that captures immediate attention.
    - **Key Audio Moments:** Descriptions of pivotal music moments.
    - **Emotional Peaks:** Points in the song where emotions intensify.
    - **Transitions and Flow:** How song sections transition and the pacing.
    - **Closing Section:** A memorable ending that leaves a lasting impression.
...
• For **each** of the six identified key elements, the Medium Panel crafts a catchy hook (total = 6).
  - This hook should be sing-worthy and is open for debate in all future refinement processes. However, it will be the core of our songs, and so take extra time for the panel to debate and discuss this step.
  - Use the attached genres for half of the created hooks to help inspire the hook, giving a subtle nod to their influence.
• Concept Panel refines all hooks once.
• Hooks must be sing‑able and act as chorus anchors; they remain open to later swaps.
...
- For each of the hooks:
  • Medium Panel selects section map ([Intro], Verse, Chorus, Bridge, Outro, etc.), runtime target, and master genre blend.
        - Using the medium panel, decide on the **song structure**, including sections like **[Intro]**, **[Verse]**, **[Chorus]**, **[Bridge]**, **[Outro]**.
        - Follow the song guide and arrangement for this step, but determine where the hook is placed within the structure as its core.
        - Determine the entire runtime in this step. Most music is around 3 minutes, but you are allowed to adjust run time as needed for your song, which you should use to guide the number of sections and amount of lyrics.

---

### Step 6 · Establish Musical & Lyrical Elements
• Concept Panel locks **genre, sub‑genres, tempo, key, instrumentation**.
• Define lyrical **themes, motifs, rhyme schemes** and target emotional arc.

---

### Step 7 · Draft Initial Music Prompts (2× per panelist)
• Each Medium Panelist writes style prompts.
• Prompts must follow MASTER guide: *Emotion → Genre → Instruments/Production → Vocalist Spec → Tempo/Key (opt) → Progression sentences → Blacklist*.
• Detail every vocalist: gender, age‑range, ethnicity, tone, rasp‑level, + 2> quirks.
      - Word craft, setting the most important elements first and the less important elements last.
      - Give the overall music style and tone.
      - You must define all vocalists and be extremely descriptive!
      - You must define very descriptive sub-genres, instruments, and pacing.

---

### Step 8 · Draft Initial Lyrics Prompts (2× per panelist)
• Each Concept Panelist writes full lyric drafts (with meta‑tags, call‑and‑response, ≤ 6 lines/30 sec, etc.).
• Embed emotion tags, sound‑effect cues, and line‑count discipline; zero plagiarism.
  - **Incorporate:**
    - **All song guide instructions**.
    - **Follow the verse structure and rhyming scheme (if given)**.
    - **Emotional resonance** and **sensory language**.
    - **Metaphors**, **imagery**, and **symbolism**.
    - **Call-and-response elements**, using parentheses for responses.
    - **Short sounds** in \* \*
    - **Long sound directions** in []
    - **Do not nest directions** - Seperate long directions in a single level, for example `[Verse 1 - A pane of glass shatters, the sound glitching into a driving beat - EMO:Anticipation - ...]`
  - **Annotate** the lyrics with **section headers** and **musical directions** in **square brackets [ ]**.
...
### Step 10 · Refinement Loop 1
• Merge best music + lyric drafts; revise for structure, originality, clarity.
  - Lyricists and composers incorporate the critics' feedback.
  - **Revise** the lyrics and musical prompts, enhancing alignment with the **style axes** and **creativity spectrum**.
  - **Ensure:**
    - Adherence to the desired **song structure**.
    - Avoiding **monotonous structures** and **unnecessary rhyming**.
    - **Clarity** and **coherence** in both lyrics and musical direction.
---

### Step 11 · Medium Panel Critics’ Review
• Assess sonic plausibility, mix clarity, vocalist fit, dynamic arc.
...
### Step 12 · Refinement Loop 2
• Incorporate feedback; enforce emotion‑first ordering, progression verbs, blacklist integrity.
  - Lyricists and composers incorporate the critics' feedback.
  - **Revise** the lyrics and musical prompts, enhancing alignment with the **style axes** and **creativity spectrum**.
  - **Ensure:**
    - Adherence to the desired **song structure** and **rhyming scheme**.
    - Avoiding **monotonous structures** and **unnecessary rhyming**.
    - Try to vary line length, intonation, and tone when it makes sense. Music is dynamic!
    - **Clarity** and **coherence** in both lyrics and musical direction.
...
### Step 14 · Refinement Loop 3
• Tweak hooks/lyrics/music for quotability and brand alignment.
  - Lyricists and composers incorporate the critics' feedback.
  - **Revise** the lyrics and musical prompts, enhancing alignment with the **style axes** and **creativity spectrum**.
  - **Ensure:**
    - Adherence to the desired **song structure** and **rhyming scheme**.
...
• All panels convene; resolve remaining gaps, ensure TOS compliance.
  ...
  - Critics offer detailed feedback, highlighting strengths and areas for improvement. Be brutual and hold nothing back. Our goal is the best possible music!

---

### Step 16 · Refinement Loop 4
• Final polish—remove helper markers, fix syllable counts, tighten phrasing.
  - Lyricists and composers incorporate the critics' feedback.
  - **Revise** the lyrics and musical prompts, enhancing alignment with the **style axes** and **creativity spectrum**.
  - **Ensure:**
    - Adherence to the desired **song structure** and **rhyming scheme**.
...
### Step 17 · Draft Song Title
• Marketing Panel proposes a compelling title (3-10 words) per surviving variant.
  - Using the Context & Marketing panel, have each panel member writes an initial song title that perfectly matches and sells the music!
---
...
### Step 19 · Refinement Loop 5
• Select final title; confirm perfect alignment with lyrics and facets.
  - Lyricists and composers incorporate the critics' feedback.
...
• Purge any residual scaffolding; validate legal/ethical clearance.
  - **Ensure:**
    - Removal of any syllable breaks or rhyme scheme letters used during composition.
    - The prompts are clear and ready for use.
    - No copyrighted material ends up in the lyrics or music prompt.
      - Avoid copyrighted work, but mention culturally important words, phrases, and sounds from diverse cultures and subcultures. We can mix national, regional, local, and even cross-cultural references like water.
...
**Step 4: Provide the Final Output**
  ...
  ```json
  {{ "music_prompts": [ {{"music_prompt": "Music Prompt 1", "lyrics_prompt": "Lyrics Prompt 1", "title":"Song 1 Title"}}, {{"music_prompt": "Music Prompt 2", "lyrics_prompt": "Lyrics Prompt 2", "title":"Song 2 Title"}}, {{"music_prompt": "Music Prompt 3", "lyrics_prompt": "Lyrics Prompt 3", "title":"Song 3 Title"}}, {{"music_prompt": "Music Prompt 4", "lyrics_prompt": "Lyrics Prompt 4", "title":"Song 4 Title"}}, {{"music_prompt": "Music Prompt 5", "lyrics_prompt": "Lyrics Prompt 5", "title":"Song 5 Title"}}, {{"music_prompt": "Music Prompt 6", "lyrics_prompt": "Lyrics Prompt 6", "title":"Song 6 Title"}} ] }}
  ```
...
- **REMEBER - Return the prompts in the following JSON format:**
