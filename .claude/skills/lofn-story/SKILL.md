---
name: lofn-story
description: Run the Lofn story/narrative pipeline (steps 00–10) backed by Claude — panel-driven prose, narrative voice, world-building, scripts. Use for stories, tales, narratives, flash fiction, scripts, or "write a story with the full pipeline". Expects a Phase-0/1 orchestrator packet from the `lofn` skill; if none exists, run `lofn` first. Do NOT use for images, music, video, or QA-only audits.
---

# Lofn Story — Claude-backed narrator pipeline

Produces narrative pieces at Lofn competition grade — distinctive voice, world-building, and emotional architecture, not generic AI prose. Depth lives in `skills/story/`; Claude is the engine (hybrid execution per `.claude/skills/lofn/EXECUTION.md`).
