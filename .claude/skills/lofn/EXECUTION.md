...
- **Music:** standalone `## 1. MUSIC PROMPT` (dense paragraph, 850–1000 chars, no bracket tag-soup, no real artist names) + separate `## EXCLUDE PROMPT`; lyrics open with `[Theme: …]` then `[SONG FORM: …]`; full EMO headers `[Section - EMO:<emotion> - <Role> - <cue>]` (emotion from `skills/lofn-core/refs/EMOTION_TAXONOMY.md`, never bare AWE/INDIGNATION); ≥1 SFX cue; 70–120 sung lines.
